...
var BrightnessSlidersFeature = class BrightnessSlidersFeature {
    load() {
    ...
    _init(displays) {
        ...
        if (displays) {
            for(var display of displays) {
          	    ...
          	    this.subSlider = new BrightnessSlider(display.bus, display.name, display.current, display.max, this);
              	this.subSliders.push(this.subSlider);
              	this.menu.addMenuItem(this.subSlider);
            }
        }
        
        this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem("Night light"));
        this.nightLightToggle = new NightLightToggle();
        this.menu.addMenuItem(this.nightLightToggle);
        this._proxy = new ColorProxy(Gio.DBus.session, C_BUS_NAME, C_OBJECT_PATH,
                                     (proxy, error) => {
                                         if (error) {
                                             log(error.message);
                                             return;
    ...
    _setBrightness(sliderValue) {
		if (!this.syncing) {
			...
			if (this.setMode == "same") {
				...
				for(var subSlider of subSliders) {
...
class NightLightToggle extends PopupMenu.PopupSwitchMenuItem {
	_init() {
		...
		this._nlsettings.bind('night-light-enabled',
            this._switch, 'state',
            Gio.SettingsBindFlags.DEFAULT);
        this._nlsettings.connect('changed::night-light-enabled', () => this._sync());
    }

    _sync() {
		this.setToggleState(this._nlsettings.get_boolean('night-light-enabled'));
	}
});
...
class BuiltinSlider extends PopupMenu.PopupImageMenuItem {
    _init(master) {
        ...
        this.add(this.slider);
        this._proxy = new BrightnessProxy(Gio.DBus.session, P_BUS_NAME, P_OBJECT_PATH,
            (proxy, error) => {
                if (error)
    ...
    _sync() {
        ...
        if (visible)
            this._setBrightness(this._proxy.Brightness / 100.0);
        this.master.sync();
    }
});
...
class NightLightSlider extends PopupMenu.PopupImageMenuItem {
    _init(options) {
        ...
        this._settings.connect('changed::night-light-temperature', () => this._sync());
        this._sync();
    }
...
class Feature extends QuickSettings.SystemIndicator {
    _init(displays) {
    	...
    	var master = new MasterSlider(displays);
